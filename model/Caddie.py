...
#         # Lít 40 deatures theo CaddieSet (Theo Figure 5)
#         # Key: Index của pha (0=Address, 7=Finish)
#         # Value: Danh sách các metrics cần lấy ở pha đó
#         self.SELECTED_FEATURES = {
#             0: ['STANCE-RATIO', 'SHOULDER-ANGLE', 'UPPER-TILT'],                                      # Address (3)
#             1: ['SHOULDER-LOC', 'UPPER-TILT', 'RIGHT-ARM-ANGLE', 'LEFT-ARM-ANGLE', 'HIP-SHIFTED', 'HIP-ROTATION', 'HEAD-LOC'],                                           # Takeaway (7)
#             2: ['SHOULDER-LOC', 'LEFT-ARM-ANGLE', 'HIP-SHIFTED', 'HIP-ROTATION', 'HEAD-LOC'],         # Backswing (5)
#             3: ['SHOULDER-LOC', 'RIGHT-LEG-ANGLE', 'HIP-SHIFTED', 'HIP-ROTATION', 'HEAD-LOC'],        # Top (5)
#             4: ['SHOULDER-HANGING-BACK', 'RIGHT-ARMPIT-ANGLE', 'HIP-SHIFTED', 'HIP-ROTATION', 'HIP-HANGING-BACK', 'HEAD-LOC'],                                      # Downswing (6)
#             5: ['SHOULDER-HANGING-BACK', 'SHOULDER-ANGLE', 'RIGHT-ARM-ANGLE', 'LEFT-ARM-ANGLE', 'HIP-SHIFTED', 'HIP-HANGING-BACK', 'HEAD-LOC'],                                       # Impact (7)
#             6: ['WEIGHT-SHIFT', 'RIGHT-LEG-ANGLE', 'LEFT-ARM-ANGLE', 'HIP-SHIFTED', 'HEAD-LOC'],      # Follow (5)
#             7: ['HIP-SHIFTED', 'FINISH-ANGLE']                                                        # Finish (2)
#         } # 40 features

#         self.norm = Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # ImageNet mean and std (RGB)
...
#         """Tính 15 metrics thô cho 1 frame"""
#         kp = self.KP
#         feats = {}
...
#         # Denormalize nếu cần (ảnh float đang normalized, cần về 0-255)
#         sequence = (
...
#         ### Chưa carry được batch size >1 do model pose inference chỉ nhận list of images

#         images_np = sequence.permute(0, 2, 3, 1).cpu().numpy()

#         # 2. Pose Estimation (Batch inference)
#         results = self.pose_model(list(images_np), verbose=False, conf=0.1)

#         # 3. Tìm Address Keypoints (Frame 0)
#         address_kp = None
#         for res in results:
...
#             print("[CaddieSetExtractor] Warning: No person detected in any frame.")
#             return np.zeros(40)

#         # 4. Tính toán và Chọn lọc 40 features
#         final_feature_vector = []

#         for i, res in enumerate(results):
#             # Lấy toàn bộ 15 metrics của frame này
#             if len(res.keypoints) > 0:
#                 k = res.keypoints.xy[0].cpu().numpy()
#                 all_15_metrics = self.calculate_all_15_metrics(k, address_kp)
#             else:
#                 all_15_metrics = self.calculate_all_15_metrics(address_kp, address_kp) # Fallback

#             # FILTERING
#             # Chỉ lấy những metric nằm trong danh sách SELECTED_FEATURES của pha này
...
#     # Test với 1 dãy giả lập (8 frames)
#     dummy_sequence = torch.randn(1, 8, 3, 160, 160)  # (B, F, C, H, W)
#     features = extractor.process_sequence(dummy_sequence)
#     print("Extracted features:", features)  # Expect (40,)
