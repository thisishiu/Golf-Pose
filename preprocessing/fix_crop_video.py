...
class RobustSmartCropper:
    ...
    def get_box_from_frame(self, frame):
        ...
        # conf=0.5: Chỉ lấy khi model khá chắc chắn đó là người
        ...
        # --- Tính toán hình vuông (Square Padding) ---
        cx = (x1 + x2) / 2
        cy = (y1 + y2) / 2
        w = x2 - x1
        h = y2 - y1
    ...
    def scan_for_crop_box(self, cap, num_samples=10):
        ...
        # Ví dụ video 100 frame, check tại: [0, 11, 22, 33, ..., 99]
        check_indices = np.linspace(0, total_frames - 1, num_samples).astype(int)
        
        print(f"Đang quét tìm golfer tại các frame: {check_indices}...")
        ...
        for idx in check_indices:
            ...
            if box is not None:
                print(f"-> Đã tìm thấy người tại frame {idx}! Box: {box}")
                return box # (Early Exit)
        
        print("-> Cảnh báo: Quét cả video vẫn không thấy người nào đủ rõ.")
        return None

    def crop_fixed_region(self, frame, box):
        """Hàm crop và resize nhanh"""
        x, y, size = box
    ...
    def process_video(self, input_path, output_path):
        ...
        print("Đang xử lý toàn bộ video...")
        while True:
            ret, frame = cap.read()
            if not ret: break
